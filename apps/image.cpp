...
static int pixel_diff(GPixel p0, GPixel p1) {
    ...
    if (da > 0) {
        std::cout << "DA: " << GPixel_GetA(p0) << " - " << GPixel_GetA(p1) << std::endl;
    }
    int dr = abs(GPixel_GetR(p0) - GPixel_GetR(p1));
    if (dr > 0) {
        std::cout << "DR: " << GPixel_GetR(p0) << " - " << GPixel_GetR(p1) << std::endl;
    }
    int dg = abs(GPixel_GetG(p0) - GPixel_GetG(p1));
    if (dg > 0) {
        std::cout << "DG: " << GPixel_GetG(p0) << " - " << GPixel_GetR(p1) << std::endl;
    }
    int db = abs(GPixel_GetB(p0) - GPixel_GetB(p1));
    if (db > 0) {
        std::cout << "DB: " << GPixel_GetB(p0) << " - " << GPixel_GetB(p1) << std::endl;
    }
