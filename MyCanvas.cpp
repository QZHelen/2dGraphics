...
/** PA 2 */
static inline float GRoundToFloat(float x) {
...
static inline void clipPolygonPoints(
    ...
    bottom_y =  GRoundToInt(p1.y());
    if (top_y != bottom_y) {
        float dY = 0.5f + GRoundToFloat(p0.y()) - p0.y();
        // float dY = 
        edges.push_back({top_y, bottom_y, p0.x() + dY * slope, slope});
    }
}
/** PA 1-2 */
/**
    ...
    */
    void drawConvexPolygon(const GPoint points[], int count, const GPaint& paint) override {
        ...
        for (int y = edges[0].top_y, i = 1; y < edges.back().bottom_y; ++y) {
            ...
            leftE.curr_x += leftE.slope;
            rightE.curr_x += rightE.slope;
        }
    }
